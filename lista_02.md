...
Compra 5 negada. Saldo insuficiente e limite de crédito excedido.A)
Compra 1 aprovada. Saldo restante: 800
...
Classe Retangulo herda FormaGeometrica:
    ...
    Método construtor (largura, altura):
        Define o valor dos atributos largura e altura com o valor passado como parâmetro.
...
Classe Circulo herda FormaGeometrica:
    ...
    Método Construtor(raio):
        Define o valor do raio com o valor passado como parâmetro.
...
var
    tempo, velocidade, velocidadeInicial, aceleracao, velocidadeMaxima, tempoMaximo, distanciaTotal: real
...
inicio
    ...
    enquanto (velocidade <= velocidadeMaxima) e (tempo <= tempoMaximo) faca
        velocidade := velocidadeInicial + aceleraçao * tempo
        ...
        se velocidade >= velocidadeMaxima entao
            escreval("O carro excedeu a velocidade máxima.")
            interromper
        fimse

        se distanciaRestante <= 0 entao
            escreval("O carro levou " + tempo + " minutos para completar a corrida")
        senao 
            escreval("O carro não conseguiu terminar a corrida dentro do tempo estipulado")
        fimse
    fimenquanto
fimalgoritmo
```
...
```
**Pseudocódigo da Multiplicação de duas Matrizes**
```
algortimo "multiplicarMatrizes"

var 
    matrizA, matrizB, matriz1, matriz2, matrizR: array

funcao multiplicarMatriz(matriz1, matriz2: array) : array
    var 
        multiplicacao: real
    inicio
        para i de 0 ate tamanhoMatriz1 passo 1 :
            matrizR[i] <- []
            para j de 0 ate tamanhoMatriz2[0] passo 1:
                multiplicacao <- 0
                para c de 0 ate tamanhoMatriz1[0] passo 1:
                    multiplicacao += matriz1[i][c] * matriz2[c][j]
                fimpara
                    matrizR[i][j] <- multiplicacao
            fimpara
                escreval(matrizR)
        fimpara
fimfuncao
...
**Código da Multiplicação de duas Matrizes**
